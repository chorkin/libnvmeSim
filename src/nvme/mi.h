 ...
 * data
 */
struct nvme_mi_aem_callbacks {
	/*This is called from inside nvme_mi_process when a payload has been validated and can be parsed.
	 *The idea is that the aem_ctx wil have some internal state of the parsing (where in the payload),
	 *and the caller can call nvme_mi_aem_get_next_event(aem_ctx, ....) to get the data for the next AE in the payload.
	 *It should continue calling this until get_next_event returns NO_MORE_DATA?!?!.
	 *When finished, the handler should return handler_next_action to indicate what shoudl be done by the core
	 *For example, a typical scenario would be to ACK when we are processing an AEM.
 ...
 * a file descriptor suitable for passing to poll()/etc.
