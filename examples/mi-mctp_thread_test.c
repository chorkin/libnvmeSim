...
int do_info(nvme_mi_ep_t ep)
{
	...
	if (rc) {
		warn("can't perform Read MI Data operation #1");
		return -1;
	}
...
void* threadFunction(void* arg) {
	...
	struct nvme_mi_read_nvm_ss_info ss_info;
	int rc = nvme_mi_mi_read_mi_data_subsys(args->ep, &ss_info);
	if (rc) {
		warn("can't perform Read MI Data operation #1");
		return -1;
	}
...
int generate_and_run_threads(nvme_root_t root)
{
    ...
    for (int i = 0; i < numThreads; i++) {
		...
		threadArgs[i].pRoot = &root;
		threadArgs[i].ep = nvme_mi_open_sim_mctp(root, 0);
